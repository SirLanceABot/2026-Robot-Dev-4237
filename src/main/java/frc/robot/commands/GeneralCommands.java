  ...
  // Put all class variables and instance variables here
    ...
    public static Command ejectAllFuelSlowlyCommand()
    {
        if(indexigator != null && accelerator != null && flywheel != null)
        {
            ...
            Commands.parallel(
            setLEDCommand(ColorPattern.kSolid, Color.kOrange),
            flywheel.burpFuelCommand().until(() -> (flywheel.isAtSetSpeed(30.0, 10.0).getAsBoolean())) // velocity that can slowy eject fuel
            .andThen(
                Commands.parallel(
    ...
    public static Command passOnTheMoveCommand(Drivetrain drivetrain, Indexigator indexigator, Accelerator accelerator, Flywheel flywheel, PoseEstimator poseEstimator)
    {
        if(drivetrain != null && indexigator != null && accelerator != null && flywheel != null && poseEstimator != null)
        {
            ...
            Commands.parallel(
                Commands.either(
                    GeneralCommands.rampUpFlywheelCommand(() -> shooterPower.getAsDouble()).until(() -> flywheel.isAtSetSpeed(shooterPower.getAsDouble(), 15).getAsBoolean()), 
                    flywheel.setControlVelocityCommand(() -> (shooterPower.getAsDouble())).until(() -> flywheel.isAtSetSpeed(shooterPower.getAsDouble(), 15).getAsBoolean()),
