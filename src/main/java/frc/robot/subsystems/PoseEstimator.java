...
public class PoseEstimator extends SubsystemBase
{
    ...
    private void configTimeOfFlightMap()
    {
        ...
        timeOfFlightMap.put(18.0, 1.50);
        timeOfFlightMap.put(19.0, 1.65); // 1.0
        timeOfFlightMap.put(20.0, 1.80); // 1.04
        timeOfFlightMap.put(21.0, 1.95);
        timeOfFlightMap.put(22.0, 2.1); // 1.22
        timeOfFlightMap.put(23.0, 2.25); // 1.29
        timeOfFlightMap.put(24.0, 2.35); // 1.39
        timeOfFlightMap.put(25.0, 2.45);
    }
    ...
    public DoubleSupplier pureLeadingAngle(Pose2d target)
    {
        return () ->
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                angleToTarget = (target == redHubPose) ? Math.atan2(yDelta, xDelta) : Math.atan2(-yDelta, -xDelta);
                distanceFromTarget = Math.hypot(Math.abs(xDelta) - Math.abs(robotCenterToShooterMeters*Math.cos(angleToTarget)), Math.abs(yDelta) - Math.abs(robotCenterToShooterMeters*Math.sin(angleToTarget)));
        ...
        for(Camera camera : cameraArray)
        {
            if(camera != null)
            {
                ...
                if(camera.getTagCount() > 0)
                {
                    ...
                    if(distToTag >= 1.75 && camera.getTagCount() < 2)
                    {
                        rejectUpdate = true;
                    }
